...
class RealtimeVisualizer:
    def __init__(self, max_points=50):
        ...
        # Setup consumer
        # Đọc từ đầu để đảm bảo thấy dữ liệu cả khi consumer khởi động muộn
        # Có thể override bằng environment variable
        import os
        # Default dùng IP Nole2 (Kafka machine)
        kafka_bootstrap = os.getenv("KAFKA_BOOTSTRAP_SERVERS", "192.168.80.51:9092")
        self.consumer = KafkaConsumer(
            'house-prices-output',
            bootstrap_servers=[kafka_bootstrap],
            group_id='viz-consumer',          # cố định group để không bỏ lỡ offset
            auto_offset_reset='earliest',     # nếu chưa có offset, đọc từ đầu
            enable_auto_commit=True,          # commit để lần sau tiếp tục từ vị trí mới
        ...
        print("=" * 60)
        print(f"✓ Đã kết nối đến Kafka: {kafka_bootstrap}")
        print("✓ Topic: house-prices-output")
    ...
    def consume_messages(self, timeout_ms=1000):
        ...
        if not messages:
            return  # Không có message, không cần log
            ...
            for record in records:
                try:
                    ...
                    if not isinstance(data, dict):
                        print(f"⚠️  Invalid message format: {type(data)}")
                        continue
                    
                    if 'id' not in data or 'actual_price' not in data or 'predicted_price' not in data:
                        print(f"⚠️  Missing required fields in message: {data.keys()}")
                        continue
                    
                    self.ids.append(data['id'])
                    self.actual_prices.append(data['actual_price'] * 100)  # Chuyển sang K$
                    self.predicted_prices.append(data['predicted_price'] * 100)
                    self.errors.append(abs(data.get('error', 0)) * 100)
